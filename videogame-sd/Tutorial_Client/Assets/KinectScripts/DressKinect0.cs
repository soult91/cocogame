...
public class DressKinect0 : MonoBehaviour {
			...
			if (manager.IsUserDetected ()) {
				...
				if (manager.IsJointTracked (userId, iJointIndex) && manager.IsJointTracked (userId, iJointIndex1)) {
					...
					if (posJoint != Vector3.zero) {
						...
						if (distancia1Y < 0.14 && distancia1Y > -0.01 && distancia1X < 0.58 && distancia1X > 0.55) {
						...
						//Checkeamos la distancia de la mano al objeto4
							...
							Debug.Log ("X: " + distancia4X);
							Debug.Log ("Y: " + distancia4Y);
						if (distancia4Y < 0.8 && distancia4Y > 0.6 && distancia4X < 3.45 && distancia4X > 3.40) {
